...
object SchemaTQ {
  ...
  def getUpgradeSchemaStep(fromSchemaVersion: Int, step: Int)(implicit logger: LoggingAdapter): DBIO[_] = {
    step match {
      ...
      case 34 => DBIO.seq(   // v2.12.0
        sqlu"alter table resourcechanges add column temp timestamp",
  ...
  val latestSchemaDescription = "changed lastupdated column in resourcechanges to timestamptz and changeid columnt to bigint"
