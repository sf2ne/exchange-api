...
object ExchangeApiTables {
  ...
  val initDB = DBIO.seq(
    (
      SchemaTQ.rows.schema ++ OrgsTQ.rows.schema ++ UsersTQ.rows.schema ++ ResourceChangesTQ.rows.schema ++ AuthenticationChangesTQ.rows.schema
  ...
  val dropDB = DBIO.seq(
    ...
    sqlu"drop table if exists nodes", sqlu"drop table if exists resourcechanges", sqlu"drop table if exists authchanges",
