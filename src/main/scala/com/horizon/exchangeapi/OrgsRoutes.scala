...
final case class ResourceChangesInnerObject(changeId: Long, lastUpdated: java.sql.Timestamp)
        ...
        complete({
          ...
          db.run(reqBody.toOrgRow(orgId).insert.asTry.flatMap({
            case Success(n) =>
              ...
              val orgChange = ResourceChangeRow(0, orgId, orgId, "org", "false", "org", ResourceChangeConfig.CREATED, ApiTime.nowUTCTimestamp)
          ...
          db.run(reqBody.toOrgRow(orgId).update.asTry.flatMap({
            case Success(n) =>
              ...
              if (n.asInstanceOf[Int] > 0) { // there were no db errors, but determine if it actually found it or not
                val orgChange = ResourceChangeRow(0, orgId, orgId, "org", "false", "org", ResourceChangeConfig.CREATEDMODIFIED, ApiTime.nowUTCTimestamp)
                orgChange.insert.asTry
              } else {
        ...
        complete({
          ...
          if (action == null) (HttpCode.BAD_INPUT, ApiResponse(ApiRespType.BAD_INPUT, ExchMsg.translate("no.valid.org.attr.specified")))
            ...
            case Success(n) =>
              ...
              if (n.asInstanceOf[Int] > 0) { // there were no db errors, but determine if it actually found it or not
                val orgChange = ResourceChangeRow(0, orgId, orgId, "org", "false", "org", ResourceChangeConfig.MODIFIED, ApiTime.nowUTCTimestamp)
                orgChange.insert.asTry
              } else {
                DBIO.failed(new DBProcessingError(HttpCode.NOT_FOUND, ApiRespType.NOT_FOUND, ExchMsg.translate("org.not.found", orgId))).asTry
              }
        ...
        db.run(OrgsTQ.getOrgid(orgId).delete.transactionally.asTry.flatMap({
          case Success(v) =>
            ...
            if (v > 0) { // there were no db errors, but determine if it actually found it or not
              val orgChange = ResourceChangeRow(0, orgId, orgId, "org", "false", "org", ResourceChangeConfig.DELETED, ApiTime.nowUTCTimestamp)
              orgChange.insert.asTry
            } else {
              DBIO.failed(new DBProcessingError(HttpCode.NOT_FOUND, ApiRespType.NOT_FOUND, ExchMsg.translate("org.not.found", orgId))).asTry
            }
...
```""", required = true, content = Array(new Content(schema = new Schema(implementation = classOf[ResourceChangesRequest])))),
  ...
  def orgChangesRoute: Route = (path("orgs" / Segment / "changes") & post & entity(as[ResourceChangesRequest])) { (orgId, reqBody) =>
    ...
    exchAuth(TOrg(orgId), Access.READ) { ident =>
      validateWithMsg(reqBody.getAnyProblem) {
        complete({
          logger.info("reqBody.lastUpdated: "+reqBody.lastUpdated)
          ...
          logger.info("orgList, orgSet: " + orgList.toString + " , " + orgSet.toString)
          ...
          logger.info("reqBodyTime: "+reqBodyTime.toString)
